...
const UserRoutesBox = () => {
  ...
  if (activeStep == 0) {
    stepContent = (
      <RouteSelectionForm
        onEditStart={(e: { target: { value: any } }) =>
          setInput({ ...input, start: e.target.value })
        }
        onEditEnd={(e: { target: { value: any } }) =>
          setInput({ ...input, end: e.target.value })
        }
      />
    );
  ...
  const startRouteLog = () => {
    axios
      ...
      .then((res: AxiosResponse) => {
        // dispatch({ type: STATE.ADD_ROUTE, payload: res.data });
      })
      .catch((error: any) => {
        console.log(error);
      });
  ...
  const endRouteLog = () => {
    axios
      ...
      .then((res: any) => {
        setLoading(dispatch);
        dispatch({ type: STATE.CONFIRM_ROUTE, payload: res.data });
        dispatch({
