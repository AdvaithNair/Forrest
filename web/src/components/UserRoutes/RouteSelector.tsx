  ...
  const useEco = () => {
    ...
    dispatch({
      ...
      payload: {
        ...
        co2saved: (
          Math.abs(
            input.fastestRoute.fuelEfficiency -
              input.bestEfficiency.fuelEfficiency
          ) *
          (state.user.carType == CAR_TYPES.ELECTRIC
            ? CO2EMISSIONS.ELECTRIC
            : CO2EMISSIONS.GAS)
        ).toFixed(2),
        duration: input.bestEfficiency.duration,
        startingLoc: input.bestEfficiency.latLon![0],
        endingLoc: input.bestEfficiency.latLon![
          input.bestEfficiency.latLon!.length - 1
        ]
      }
    });
  ...
  const useFast = () => {
    ...
    dispatch({
      ...
      payload: {
        ...
        endingLoc: input.fastestRoute.latLon![
          input.fastestRoute.latLon!.length - 1
        ]
      }
    });
    clearLoading(dispatch);
      ...
      >
                  ...
                  <Card style={{ backgroundColor: '#DDDDDD' }}>
                    <CardContent>
                      ...
                      <Typography variant='body2'>
                        {input.fastestRoute.route}
                      </Typography>
                      <Typography variant={'body2'} color={'textSecondary'}>
                        Duration: {input.fastestRoute.duration.toFixed(2)}{' '}
                        minutes
                      </Typography>
                      <Typography variant={'body2'} color={'textSecondary'}>
                        CO2 Savings:{' '}
                ...
                >
                  <Card style={{ backgroundColor: '#DDDDDD' }}>
                    <CardContent>
                      <Avatar
                        style={{
                          backgroundColor: ecoRoute ? COLORS.SECONDARY : 'grey',
                          borderRadius: 5,
                          width: 35,
                          height: 35,
                          display: 'flex',
                      ...
                      <Typography variant='body2'>
                        {input.bestEfficiency.route}
                      </Typography>
                      <Typography variant={'body2'} color={'textSecondary'}>
                        Duration: {input.bestEfficiency.duration.toFixed(2)}{' '}
                        minutes
                      </Typography>
                      <Typography variant={'body2'} color={'textSecondary'}>
                        CO2 Savings:{' '}
        ...
        <Grid item sm>
          <DirectionBar
            ...
            directionList={
              ecoRoute
                ? input.bestEfficiency.directions
                : input.fastestRoute.directions
            }
            onForward={directionForward}
            onBackward={directionBackward}
          />
          <BasicMap
            ecoMode={ecoRoute}
            currentDirection={directionIndex}
            polySet={
              ecoRoute ? input.bestEfficiency.latLon : input.fastestRoute.latLon
            }
          />
        </Grid>
      </Grid>
